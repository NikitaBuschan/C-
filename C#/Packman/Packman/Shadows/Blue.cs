...
{
    public class BlueShadow : Unit
    {
        RememberStepObject eat = null;

        public BlueShadow(int x, int y)
        {
            UnitNumber = 7;
            Coord = new Coord(x, y);
            NextCoord = new Coord(x, y);
            Sprite = new Sprite(ConsoleColor.Blue, "%");
        }
        ...
        public override void Eat() => Coord = Create.Blue().Coord;

        public override void Exit()
        {
            
            var temp = new Coord(0, 0);
            if (Coord == new Coord(14, 14))
            {
