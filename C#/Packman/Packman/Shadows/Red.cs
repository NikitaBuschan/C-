...
{
    public class RedShadow : Unit
    {
        RememberStepObject eat = null;

        public RedShadow(int x, int y)
        {
            UnitNumber = 6;
            Coord = new Coord(x, y);
            NextCoord = new Coord(x, y);
            Sprite = new Sprite(ConsoleColor.Red, "%");
        }
        ...
        public override void Eat() => Coord = Create.Red().Coord;

        public override void Exit()
        {
            var temp = new Coord(0, 0);
            if (Coord == new Coord(14, 15))
            {
                temp = new Coord(13, 15);
                Draw.RedrowUnitToNewPosition(this, temp);
                Coord = temp;
            }
            else if (Coord == new Coord(13, 15))
            {
