...
{
    public class GreenShadow : Unit
    {
        RememberStepObject eat = null;

        public GreenShadow(int x, int y)
        {
            UnitNumber = 9;
            Coord = new Coord(x, y);
            NextCoord = new Coord(x, y);
            Sprite = new Sprite(ConsoleColor.Green, "%");
        }
        ...
        public override void Eat() => Coord = Create.Green().Coord;

        public override void Exit()
        {
            var temp = new Coord(0, 0);
            if (Coord == new Coord(14, 12))
            {
                temp = new Coord(13, 12);
                Draw.RedrowUnitToNewPosition(this, temp);
                Coord = temp;
            }
            else if (Coord == new Coord(13, 12))
            {
